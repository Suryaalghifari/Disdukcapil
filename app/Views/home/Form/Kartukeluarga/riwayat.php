...
<section class="services" id="history">
    <div class="container">
        <div class="section-header fade-in visible">
            <h2 class="section-title">Riwayat Pengajuan Kartu Keluarga</h2>
            <p class="section-subtitle">
                Lihat semua pengajuan Kartu Keluarga Anda.<br>
                Klik <b>Download</b> jika file KK sudah tersedia.
            </p>
        </div>
        ...
        <div class="services-grid">
            <?php if (empty($riwayat)): ?>
                <div class="service-card fade-in visible" style="text-align:center;">
                    <span>Belum ada riwayat pengajuan Kartu Keluarga.</span>
                </div>
            <?php else: ?>
                    ...
                    <div class="service-card fade-in visible">
                        <div style="margin-bottom: 12px;">
                            <div style="font-weight: 600; font-size: 1rem; color: #e11d48;">
                                <?= esc($pengajuan['nama_kepala_keluarga']) ?>
                            </div>
                            <?php if (!empty($pengajuan['nik_kepala_keluarga'])): ?>
                                <div style="font-size: 0.95rem; color: #777;">
                                    NIK: <?= esc($pengajuan['nik_kepala_keluarga']) ?>
                                </div>
                            <?php endif; ?>
                        </div>
                        <div style="font-size: 0.95rem; color: #555; margin-bottom: 6px;">
                            Alamat: <?= esc($pengajuan['alamat']) ?>, RT <?= esc($pengajuan['rt']) ?>/RW <?= esc($pengajuan['rw']) ?>, <?= esc($pengajuan['kelurahan']) ?>, <?= esc($pengajuan['kecamatan']) ?>, <?= esc($pengajuan['kode_pos']) ?>
                        </div>
                        <div style="font-size: 0.95rem; color: #555; margin-bottom: 6px;">
                        ...
                        <?php if (!empty($pengajuan['file_pdf'])): ?>
                            <a href="<?= site_url('form/kartukeluarga/download/' . $pengajuan['id']) ?>"
