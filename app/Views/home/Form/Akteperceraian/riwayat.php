...
<section class="services" id="history">
    <div class="container">
        <div class="section-header fade-in visible">
            <h2 class="section-title">Riwayat Pengajuan Akte Perceraian</h2>
            <p class="section-subtitle">
                Lihat semua pengajuan Akte Perceraian Anda.<br>
        ...
        <div class="services-grid">
            <?php if (empty($riwayat)): ?>
                <div class="service-card fade-in visible" style="text-align:center;">
                    <span>Belum ada riwayat pengajuan Akte Perceraian.</span>
                </div>
            <?php else: ?>
                <?php foreach ($riwayat as $pengajuan): ?>
                    <div class="service-card fade-in visible">
                        ...
                        <div style="font-size: 0.95rem; color: #555; margin-bottom: 6px;">
                            Nomor Perkawinan: <?= esc($pengajuan['nomor_perkawinan']) ?><br>
                            Tanggal Perkawinan: <?= esc(date('d-m-Y', strtotime($pengajuan['tanggal_perkawinan'] ?? 'now'))) ?><br>
                            Tempat Perkawinan: <?= esc($pengajuan['tempat_perkawinan']) ?>
                        </div>
                        <div style="font-size: 0.95rem; color: #555; margin-bottom: 6px;">
                            Tanggal Perceraian: <?= esc(date('d-m-Y', strtotime($pengajuan['tanggal_perceraian'] ?? 'now'))) ?><br>
                            Tempat Perceraian: <?= esc($pengajuan['tempat_perceraian']) ?>
                        </div>
                        <div style="font-size: 0.95rem; color: #555; margin-bottom: 6px;">
                            Penyebab Perceraian: <?= esc($pengajuan['penyebab_perceraian']) ?>
                        </div>
                        <div style="font-size: 0.95rem; color: #555; margin-bottom: 6px;">
                        ...
                        <?php if (!empty($pengajuan['file_pdf'])): ?>
                            <a href="<?= site_url('form/akteperceraian/downloadAkte/' . $pengajuan['id']) ?>"
